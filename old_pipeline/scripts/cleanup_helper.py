...
def coops_utils_cleanup(
    util_shp: gpd.GeoDataFrame, state_csv_df: pd.DataFrame, consts: dict
) -> gpd.GeoDataFrame:
    """
    ...
    try:
        coops_utils = helper.data_preprocess(
            input_df=coops_utils,
            cols_drop=consts.drop_cols,
    ...
    util_merged = coops_utils.merge(
        gpd.GeoDataFrame(state_df), how="left", on=consts.st_code
    )
    util_merged[consts.area] = np.round(util_merged[consts.geo].area, 4)
    util_merged = util_merged[
        util_merged[consts.filter_col].isin(consts.filters)
    ].to_crs(epsg=3857)
    util_merged[consts.new_col] = util_merged[consts.filter_col].apply(
        lambda x: consts.filters_new[1]
        if x == consts.filters[2]
        else consts.filters_new[0]
    )

    rural_coops = util_merged.query(
        f'{consts.new_col} == "{consts.filters_new[1]}"'
    ).to_crs(epsg=3857)
    municipal_utils = util_merged.query(
        f'{consts.new_col} == "{consts.filters_new[0]}"'
    ).to_crs(epsg=3857)
...
def j40_cleanup(j40_shp: gpd.GeoDataFrame, consts: dict) -> gpd.GeoDataFrame:
    ...
    """
    try:
        j40 = helper.data_preprocess(
            input_df=j40_shp,
            cols_to_keep=consts.cols_to_keep,
            cols_rename=consts.rename_cols,
        )
    except:
        ...
        raise ValueError("Invalid column names in the input shapefile.")
    j40 = j40[
        (j40[consts.filter_cols[0]] == 1)
        & (j40[consts.filter_cols[1]].is_valid == True)
    ]
...
def energy_cleanup(
    coal_shp: gpd.GeoDataFrame, ffe_shp: gpd.GeoDataFrame, consts: dict
) -> gpd.GeoDataFrame:
    """
    ...
    """
    try:
        coal_df = helper.data_preprocess(
            input_df=coal_shp,
            cols_drop=consts.coal_drop_cols,
        ...
        raise ValueError("Invalid column names in the input shapefile.")
    try:
        ffe_df = helper.data_preprocess(
            input_df=ffe_shp,
            cols_drop=consts.ffe_drop_cols,
...
def cty_st_borders_cleanup(
    county_df: gpd.GeoDataFrame,
    st_fips_csv: pd.DataFrame,
    ...
    """
    try:
        states_fips = helper.data_preprocess(
            input_df=st_fips_csv.copy(),
            cols_to_keep=consts.cols_read_csv,
    ...
    try:
        st_df = helper.data_preprocess(
            input_df=st_df,
            cols_drop=consts.st_drop_cols,
...
def dci_cleanup(
    dci_csv_df: pd.DataFrame, zip_shp: gpd.GeoDataFrame, consts: dict
) -> gpd.GeoDataFrame:
    """
    ...
    dci_shp = dci_csv_df.merge(
        zip_shp, left_on=consts.zip, right_on=consts.geoid, how="left"
    )
    try:
        dci_shp = helper.data_preprocess(
            input_df=dci_shp.copy(),
            cols_to_keep=consts.cols_to_keep,
        ...
        raise ValueError("Invalid column names in the input shapefile.")
    dci_shp = dci_shp[
        ~dci_shp[consts.geo].isnull()
    ]  # all the 4 null zips have quintile scores not equal to 5
...
class LowIncomeCleanup:
    ...
    def _load_preprocess_data(self, path, cols_to_keep, cols_rename, data_type="csv"):
        ...
        data = helper.data_preprocess(
            input_df=data.copy(), cols_to_keep=cols_to_keep, cols_rename=cols_rename
        )
        if data_type == "csv":
    ...
    def _merge_geodataframes(self, gdf1, gdf2, on=None, left_on=None, right_on=None):
        ...
        if on is not None:
            return gpd.GeoDataFrame(
                pd.merge(gdf1, gdf2, on=on, how="left"), geometry=self.consts.geo
            )
        elif left_on is not None and right_on is not None:
            return gpd.GeoDataFrame(
                pd.merge(gdf1, gdf2, left_on=left_on, right_on=right_on, how="left"),
    ...
    def _spatial_join(self, gdf1, gdf2, predicate="intersects"):
        ...
        """
        return gpd.sjoin(gdf1, gdf2, how="left", predicate=predicate).drop(
            columns=["index_right"]
        )
    ...
    def _apply_low_income_conditions(self, gdf):
        ...
        gdf.loc[
            gdf[self.consts.li_conds.pov_perc] > 20, self.consts.li_conds.li_col
        ] = "Low Income"
        gdf.loc[
            gdf[self.consts.li_conds.med_inc]
            < gdf[self.consts.li_conds.st_med_inc] * 0.8,
            self.consts.li_conds.li_col,
        ] = "Low Income"
        gdf.loc[
            (gdf[self.consts.li_conds.lsad] == "M1")
            & (
                gdf[self.consts.li_conds.med_inc]
                < gdf[self.consts.li_conds.msa_medInc] * 0.8
            ),
            self.consts.li_conds.li_col,
        ] = "Low Income"
        gdf.loc[
            gdf[self.consts.li_conds.li_col] == "", self.consts.li_conds.li_col
        ] = "Not Low Income"
    ...
    def clean_data(
        ...
        """
        # Load the data
        pov_df = self._load_preprocess_data(
            pov_csv_path,
            cols_to_keep=self.consts.pov_cols_to_keep,
            cols_rename=self.consts.pov_renamed_cols,
        )
        li_tract_df = self._load_preprocess_data(
            li_tract_csv_path,
            cols_to_keep=self.consts.li_tract_csv_cols_to_keep,
            cols_rename=self.consts.li_tract_csv_renamed_cols,
        )
        li_st_df = self._load_preprocess_data(
            li_st_csv_path,
            cols_to_keep=self.consts.st_csv_cols_to_keep,
            cols_rename=self.consts.st_csv_renamed_cols,
        )
        li_msa_df = self._load_preprocess_data(
            li_msa_csv_path,
            cols_to_keep=self.consts.msa_csv_cols_to_keep,
            cols_rename=self.consts.msa_csv_renamed_cols,
        )
        msa_shp_df = self._load_preprocess_data(
            msa_shp_path,
            cols_to_keep=self.consts.msa_shp_cols_to_keep,
            cols_rename=self.consts.msa_shp_renamed_cols,
            data_type="shp",
        )
        all_tracts_shp = helper.load_data(
            tracts_shp_path, type="shp"
        )  # Load the census tracts shapefile

        pov_df[self.consts.pov_perc] = (
            pov_df[self.consts.pov_pop] / pov_df[self.consts.pov_tot_pop]
        )
        msa_shp_df[self.consts.msa_shp_cbsaId] = msa_shp_df[
            self.consts.msa_shp_cbsaId
        ].astype(int)
        # Merge the datasets together
        inc_merged_csv = self._merge_dataframes(
            li_tract_df, li_st_df, on=self.consts.state
        )  # Tract + State income data
        msa_merge_shp = self._merge_geodataframes(
            li_msa_df,
            msa_shp_df[self.consts.msa_shp_merge_cols],
            on=self.consts.msa_shp_cbsaId,
        )  # MSA + MSA shapefile

        inc_merged_shp = self._merge_geodataframes(
            inc_merged_csv[self.consts.merge_all.inc_merged_cols],
            all_tracts_shp[self.consts.merge_all.tract_shp_cols],
            left_on=self.consts.merge_all.leftid,
            right_on=self.consts.merge_all.rightid,
        )  # Inc Tract + Tract shapefile
        inc_merged_shp.rename(
            columns=self.consts.merge_all.inc_merged_cols_renamed, inplace=True
        )

        inc_pov_merged = self._merge_dataframes(
            inc_merged_shp,
            pov_df[self.consts.merge_all.pov_cols],
            on=self.consts.merge_all.leftid,
        )  # Inc Tract+Tract shapefile+Poverty data
        inc_pov_merged.drop(columns=self.consts.merge_all.pov_drop_cols, inplace=True)

        msa_merge_shp = helper.data_preprocess(
            input_df=msa_merge_shp.copy(),
...
class PopulationCleanup:
    ...
    def _merge_with_shapefile(self, clean_df, shp_df, merge_key):
        ...
        merged_df = pd.merge(
            clean_df,
            shp_df[merge_config.shp_cols],
            left_on=merge_config.leftid,
            right_on=merge_config.rightid,
            how="left",
        )
        merged_df = gpd.GeoDataFrame(merged_df, geometry=self.consts.geo).to_crs(
            epsg=self.consts.crs
        )
        return merged_df

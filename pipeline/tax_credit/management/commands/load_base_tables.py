...
from tax_credit.models import CensusBlockGroup, CensusTract, GeographyType, Program
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        jobs = [
            geography_type_load_job,
            program_load_job,
            census_tract_load_job,
            census_block_group_load_job,
        ]
        for job in jobs:
            ...
            objs = (
                ...
                for row in reader.iterate(job.file_name, delimiter=job.delimiter)
            )
            while (
                True

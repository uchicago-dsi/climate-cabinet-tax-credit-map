...
from tax_credit.load_job import FileLoadJob
from tax_credit.models import Geography, GeographyType, GeographyTypeProgram, Program
...
class Command(BaseCommand):
    """
    Loads data into Geography_Type_Program and Geography tables. These tables have 1 level of
    dependency on other loaded tables.
    """

    help = (
        "Loads data to populate the Geography_Type_Program and Geography tables. "
        "`load_base_tables` must run first."
    )

    def add_arguments(self, parser: CommandParser) -> None:
        """
        --skip-file : takes a list of space separated file names which will not be loaded
        """
        parser.add_argument("--skip-file", nargs="+", default=[])

    def handle(self, *args, **options) -> None:
        """
        This funciton builds load jobs and loads data for all tables that have a dependency on one
        of the base tables. The tables loaded here, Geography_Type_Program and Geography have the
        base tables as direct dependencies.
        """
        skip_files = options["skip_file"]

        logger.info(
            "Loading dependent tables tables: geographies and geography-program match, and census "
            "tracts"
        )
        ...
        jobs = filter(
            lambda j: j.file_name not in skip_files,
            [
        ...
        for job in jobs:
            ...
            while True:  # See django docs here for pattern,
    ...
    @staticmethod
    def recycle_connection(job: FileLoadJob):
        """
        Recycles the connection from the model from the previous job. Without this the connection
        will expire and cause SSL errors when working with Neon Postgres.
        """

...
class Validator:
    ...
    def file_exists(load_job: LoadJob, data_reader: DataReader):
        ...
        if not filename in bucket_contents:
            raise RuntimeError(
                f"Requeste file is missing from data folder : {filename} . Available files : {bucket_contents} ."
            )
    ...
    def cols_exist(load_job: LoadJob, data_reader: DataReader):
        ...
        if missing_cols:
            raise RuntimeError(
                f"Columns are missing from the file : {missing_cols} . Available fields : {actual_cols} ."
            )

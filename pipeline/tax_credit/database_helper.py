...
class DatabaseHelper:
    @staticmethod
    def load_batched(
        job: LoadJob, reader: IterativeDataReader, load_batch_size, batch_number_of
    ):
        ...
        try:
            ...
            while True:
                ...
                logger.info(
                    f"{job.job_name} batch {batch_ct} - Starting, size {batch_size}"
                )
                ...
                average_proc_time_per_record = (
                    (1 - smoothing_factor) * average_proc_time_per_record
                    + smoothing_factor * processing_time / batch_size
                )
                if batch_ct < 5:
                    batch_size = min(
                        ceil(target_time / average_proc_time_per_record),
                        ceil(batch_size * 2),
                    )
                else:
                    batch_size = ceil(target_time / average_proc_time_per_record)
                logger.debug(
                    f"{job.job_name} batch {batch_ct} - Processing time : {processing_time}"
                )

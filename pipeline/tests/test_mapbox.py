...
class TestTilesetSources(unittest.TestCase):
    ...
    def test_create_or_append(self):
        """Assert that creating a tileset does not raise an exception.
        """
    ...
    def test_delete(self):
        """Assert that deleting a tileset does not raise an exception
        """
        id = TestTilesetSources._source_id
        TestTilesetSources._client.delete_tileset_source(id)

    def test_list(self):
        """Assert that listing Mapbox tileset sources does not
...
class TestTilesets(unittest.TestCase):
    ...
    def test_lifecycle(self):
        ...
        matches = [
            t 
            for t in tilesets 
            if t["id"].split(".")[-1] == tileset_formal_name
        ]
        if not matches:

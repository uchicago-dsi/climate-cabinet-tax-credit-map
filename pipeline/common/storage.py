    ...
    def get() -> FileSystemHelper:
        if not FileSystemHelperFactory._fileSystemHelper:
            ...
            if env == "DEV":
                ...
                raise RuntimeError(
                    "Unable to instantiate FileSystemHelper, invalid environment variable passed ",
                    f"for 'ENV'. Value passed : {env} .",
                )
...
class DataReaderFactory:
    ...
    def get(type: str) -> DataReader:
        if not type:
            raise TypeError(
                "A value of { csv, parquet, geoparquet } must be given to DataReaderFactory for "
                "type"
            )
